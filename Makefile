...
.PHONY: help all run build deps fmt fmt-check imports lint vet staticcheck golangci-lint test migrate-up migrate-down docker-build docker-run docker-stop docker-logs docker-compose-up docker-compose-down preprod vuln tag-create tag-push tag-release ssh-vps
